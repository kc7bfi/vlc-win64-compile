...
wget ftp://ftp.uvsq.fr/pub/gcc/releases/gcc-$GCC_VERSION/gcc-$GCC_VERSION.tar.xz
...
make install
cd /build
wget -q http://www.mpfr.org/mpfr-$MPFR_VERSION/mpfr-$MPFR_VERSION.tar.gz
wget -q https://gmplib.org/download/gmp/gmp-$GMP_VERSION.tar.xz
wget -q ftp://ftp.gnu.org/gnu/mpc/mpc-$MPC_VERSION.tar.gz
tar xf mpfr-$MPFR_VERSION.tar.gz || { echo "Download mpfr-$MPFR_VERSION" ; exit 1; }
tar xf gmp-$GMP_VERSION.tar.xz || { echo "Download gmp-$GMP_VERSION" ; exit 1; }
tar xf mpc-$MPC_VERSION.tar.gz || { echo "Download mpc-$MPC_VERSION" ; exit 1; }
...
make all-gcc || { echo 'Make all-gcc failed' ; exit 1; }
ls
make install-gcc
